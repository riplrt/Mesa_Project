...
#   for all primary inflammatory biomarkers × race/ethnicity on the absolute
#   risk-difference scale via Cox predicted risks, with delta-method CIs.
# - Section 18: Formal causal mediation via CMAverse with sensitivity analysis
#   for unmeasured mediator–outcome confounding (E-value), replacing the
#   sequential-adjustment approximation with counterfactual decomposition
...
# =============================================================================
# NEW SECTION 17: RERI Additive Interaction Analysis
# =============================================================================
# Reference: VanderWeele TJ & Knol MJ (2014). A tutorial on interaction.
#            Epidemiologic Methods, 3(1), 33-72.
#
# Rationale: Section 8 tests multiplicative interaction (HR ratio scale) via
# LRT.  Equity-science reviewers require RERI (Relative Excess Risk due to
# Interaction) on the absolute risk-difference scale, because multiplicative
# and additive interaction can diverge.
#
# Method:
#   For each biomarker × race contrast (non-White vs White), the exposure is
#   dichotomised at the median (high vs low).  Four joint strata are created:
#     A=0,B=0 : White + low biomarker   (reference)
#     A=1,B=0 : Non-White + low biomarker
#     A=0,B=1 : White + high biomarker
#     A=1,B=1 : Non-White + high biomarker
#
#   RERI = HR_11 - HR_10 - HR_01 + 1
#   with delta-method 95% CI from the covariance matrix.
#
#   RERI > 0 → positive additive interaction (super-additive risk).
#   RERI < 0 → sub-additive (one factor partially compensates the other).
# =============================================================================
...
compute_reri_for_biomarker <- function(data, biomarker, base_covars,
                                       ...
                                       race_var  = "race_eth",
                                       ref_race  = "1") {
  # --- Dichotomise biomarker at median (high = 1) ---
  d <- data %>%
  ...
  # --- Fit Cox with joint indicator + covariates ---
  rhs_base <- if (length(base_covars) > 0) paste(base_covars, collapse = " + ") else NULL
  ...
  # Names produced by R for factor "joint" with ref = A0B0
  ...
  # --- Delta-method variance for RERI ---
  # Gradient vector: d(RERI)/d(beta) for the three joint coefficients
  grad <- c(
    -exp(b10),   # d/d(b10) = -HR_10
    -exp(b01),   # d/d(b01) = -HR_01
     exp(b11)    # d/d(b11) = +HR_11
  )
...
# Run RERI for each primary inflammatory biomarker
...
# =============================================================================
# NEW SECTION 18: Formal Causal Mediation via CMAverse
# =============================================================================
...
#
# Goal:
#   Decompose the total effect of race/ethnicity (exposure) on incident HF
#   (outcome) through inflammatory biomarkers (mediator) into:
#     - Natural Direct Effect (NDE)
#     - Natural Indirect Effect (NIE = mediated portion)
#   Using the counterfactual framework.
#
# Sensitivity: E-value for unmeasured mediator-outcome confounding.
#
# We use a survival outcome (Cox/AFT) with CMAverse's regression-based
# approach, which handles survival mediators properly.
# =============================================================================

cat("\n========== Section 18: Formal Causal Mediation (CMAverse) ==========\n")
...
# --- Prepare analytic dataset for mediation ---
# Exposure: race_nonwhite (binary: 0 = White, 1 = non-White)
#   - We also run contrasts for Black (race1c=3) vs White (race1c=1) specifically,
#     since this is the primary equity-science comparison.
# Mediator: each inflammatory biomarker (continuous z-scored)
# Outcome: incident HF (survival: hf_time_days, hf_event)
# Confounders: age, sex, site, BMI, SBP, HTN, DM, lipids, eGFR, education,
#              income, smoking (same base_covars as Section 8)
...
mesa_mediation <- mesa_main %>%
  dplyr::mutate(
    ...
    # Convert time to years for AFT models / interpretability
    hf_time_years = hf_time_days / 365.25
  )

# Identify confounders available in the data (character vector for CMAverse)
mediation_confounders <- base_covars[base_covars %in% names(mesa_mediation)]

# --- Helper: run CMAverse for one exposure × one mediator ---
run_cmaverse_mediation <- function(data, exposure, mediator,
                                   confounders,
                                   time_var  = "hf_time_years",
                                   event_var = "hf_event",
                                   nboot = 500,
                                   seed  = 42) {
  # CMAverse requires complete cases for the variables used
  vars_all <- c(exposure, mediator, confounders, time_var, event_var)
  d <- data %>%
    dplyr::select(dplyr::any_of(vars_all)) %>%
    dplyr::filter(stats::complete.cases(.), .data[[time_var]] > 0)

  if (nrow(d) < 50L || sum(d[[event_var]], na.rm = TRUE) < 10L) {
    return(tibble(
      exposure    = exposure,
      mediator    = mediator,
      n           = nrow(d),
  ...
  # CMAverse cmest() call
  # yreg = "survCox" for Cox PH outcome model

  # mreg = "linear" for continuous mediator
  # Use bootstrap for CIs (robust to non-normality)
  res <- tryCatch({
    set.seed(seed)
    CMAverse::cmest(
      ...
      mreg       = list("linear"),        # mediator regression
      yreg       = "survCox",             # outcome regression (Cox PH)
      astar      = 0,                     # reference exposure level
      a          = 1,                     # active exposure level
      mval       = list(0),              # mediator value for NDE (mean = 0 since z-scored)
      estimation = "imputation",          # parametric g-computation
      inference  = "bootstrap",
      nboot      = nboot
    )
  }, error = function(e) {
    cat("  CMAverse error for", exposure, "->", mediator, ":", conditionMessage(e), "\n")
    NULL
  })
  ...
  if (is.null(res)) {
    return(tibble(
      ...
      n = nrow(d), events = sum(d[[event_var]], na.rm = TRUE),
  ...
  # Extract summary
  s <- tryCatch(summary(res), error = function(e) NULL)
  if (is.null(s)) {
    return(tibble(
      exposure = exposure, mediator = mediator,
      n = nrow(d), events = sum(d[[event_var]], na.rm = TRUE),
  ...
  # CMAverse summary produces a matrix of effect estimates
  # Row names typically: "pnde", "tnie", "tnde", "pnie", "te", "pm"
  # We extract pure natural direct effect (pnde) and total natural indirect (tnie)
  ef <- as.data.frame(s$effect.decomposition)

  safe_extract <- function(rowname, colname) {
    if (rowname %in% rownames(ef) && colname %in% colnames(ef)) {
      as.numeric(ef[rowname, colname])
    } else {
      NA_real_
    }
  }
  ...
  tibble(
    ...
    n           = nrow(d),
    events      = sum(d[[event_var]], na.rm = TRUE),
    nde_est     = safe_extract("pnde", "Estimate"),
    nde_lcl     = safe_extract("pnde", "2.5%"),
    nde_ucl     = safe_extract("pnde", "97.5%"),
    nde_p       = safe_extract("pnde", "Pr(>|z|)"),
    nie_est     = safe_extract("tnie", "Estimate"),
    nie_lcl     = safe_extract("tnie", "2.5%"),
    nie_ucl     = safe_extract("tnie", "97.5%"),
    nie_p       = safe_extract("tnie", "Pr(>|z|)"),
    te_est      = safe_extract("te", "Estimate"),
    te_lcl      = safe_extract("te", "2.5%"),
    te_ucl      = safe_extract("te", "97.5%"),
    pm_est      = safe_extract("pm", "Estimate"),
    pm_lcl      = safe_extract("pm", "2.5%"),
    pm_ucl      = safe_extract("pm", "97.5%"),
    status      = "success"
  )
}

# --- Run mediation for each inflammatory biomarker ---
# Primary contrast: Black vs White (the most policy-relevant disparity)
cat("\n--- CMAverse: Black vs White, through each inflammatory biomarker ---\n")
...
# Secondary contrast: any non-White vs White (broader)
cat("\n--- CMAverse: Non-White vs White, through each inflammatory biomarker ---\n")
...
# (on the risk-ratio scale) needed to fully explain away the NIE.
...
evalue_results <- mediation_all %>%
  ...
  dplyr::rowwise() %>%
  dplyr::mutate(
    # NIE is on log-HR scale from Cox; convert to HR for E-value
    nie_hr     = exp(nie_est),
    nie_hr_lcl = exp(nie_lcl),
    nie_hr_ucl = exp(nie_ucl),
    # E-value for point estimate
    evalue_point = tryCatch({
      ev <- EValue::evalues.HR(est = nie_hr, lo = nie_hr_lcl, hi = nie_hr_ucl,
                                rare = TRUE)  # rare disease assumption OK for HF incidence
    ...
    evalue_ci = tryCatch({
      ev <- EValue::evalues.HR(est = nie_hr, lo = nie_hr_lcl, hi = nie_hr_ucl,
                                rare = TRUE)
      # The lower CI E-value (for a protective or risk NIE)
      lb <- as.numeric(ev["E-values", "lower"])
      lb
    }, error = function(e) NA_real_)
  ) %>%
  dplyr::ungroup() %>%
  dplyr::transmute(
    exposure = exposure_label,
    mediator = biomarker_label,
    nie_hr   = formatC(nie_hr, format = "f", digits = 3),
    `NIE HR (95% CI)` = glue::glue(
      "{formatC(exp(nie_est), format = 'f', digits = 3)} ",
      "({formatC(exp(nie_lcl), format = 'f', digits = 3)}, ",
      "{formatC(exp(nie_ucl), format = 'f', digits = 3)})"
    ),
...
cat("Section 18 outputs (CMAverse mediation):\n")
cat("  - mesa_hf_causal_mediation_cmaverse.csv\n")
