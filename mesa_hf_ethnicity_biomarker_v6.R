...
#            and event arguments — NOT a pre-built Surv() column. Removed the
#            surv_outcome column from prepare_mediation_data(); now returns
#            time_var and event_var separately, passed as outcome= and event=
#            to cmest(). Runner renamed to run_cmaverse_mediation_v6().
#------------------------------------------------------------------------------
...
# (Identical to v4 — no changes needed)
# =============================================================================
...
# =============================================================================
# SECTION 18 (v6 REWRITE): Formal Causal Mediation via CMAverse
# =============================================================================
# Reference: Shi B, Choirat C, Coull BA, VanderWeele TJ, Valeri L (2021).
#            CMAverse: A suite of functions for reproducible causal mediation
#            analyses. Epidemiology, 32(5), e20-e22.
#
# v4 FAILURE ROOT CAUSES (all fixed in v5 and v6):
#   (a) cmest() outcome specification — fixed across v5/v6 (see below).
#   (b) Factor confounders (sex, site_factor, cig1c) passed as character names
#       in basec without basecval causes internal formula-builder crash.
#   (c) tryCatch swallowed error messages; they were never persisted to CSV.
#
# v5 FIXES:
#   FIX (a): Pre-built Surv() column as outcome= — this turned out to be wrong;
#            see v6 fix below.
#   FIX (b): Expand all factor confounders to numeric 0/1 dummies via
#            model.matrix() BEFORE passing to cmest(). Only numeric columns
#            appear in basec — no factor columns at all.
#   FIX (c): Capture conditionMessage(e) into the output tibble so the
#            actual R error is written to the CSV for any future debugging.
#   FIX (d): Use estimation="paramfunc" (parametric functional) as the primary
#            estimator and fall back to "imputation" only if needed.
#   FIX (e): Set mval to list(median) of the mediator instead of list(0).
#
# v5 REMAINING BUGS (fixed in v6):
#   Bug 1: yreg = "survCox" is not a valid CMAverse value; caused universal
#          model_error. Corrected to yreg = "coxph".
#   Bug 2: cmest() with yreg = "coxph" requires separate outcome= (time column)
#          and event= (event indicator column) arguments. The pre-built Surv()
#          column strategy from v5 FIX (a) was incorrect for coxph. Removed.
#
# v6 FIXES:
#   Fix A: yreg = "coxph"  (was "survCox")
#   Fix B: outcome = time_var (e.g. "hf_time_years") + event = event_var
#          (e.g. "hf_event") — NOT a pre-built Surv() column name.
# =============================================================================

cat("\n========== Section 18 (v6): Formal Causal Mediation (CMAverse) ==========\n")
...
# =============================================================================
# FIX (b): Helper to expand factor columns to numeric dummies
# =============================================================================
expand_factors_to_dummies <- function(data, factor_cols) {
  # For each factor column, create k-1 dummy columns (treatment contrast)
  # and return updated data frame + vector of new numeric column names
  new_cols <- character()
  ...
  for (fc in factor_cols) {
    ...
    if (is.factor(col) || is.character(col)) {
      ...
      # Reference = first level; create dummies for levels 2..k
      for (i in 2:length(lvls)) {
        dummy_name <- paste0(fc, "_", lvls[i])
        # Sanitize name for R formula safety
...
# =============================================================================
# Prepare data: expand factor confounders to dummies, drop incomplete cases
# =============================================================================
# v6 fix: Do NOT pre-build a Surv() column. CMAverse cmest() with yreg = "coxph"
# expects outcome = <time column name> and event = <event column name> as
# separate character arguments. A pre-built Surv() column causes a model error.

prepare_mediation_data <- function(data, exposure, mediator, confounders,
                                   time_var = "hf_time_years",
                                   event_var = "hf_event") {
  # Identify which confounders are factor/character
  ...
  # Keep only needed columns + complete cases
...
# =============================================================================
# Main mediation runner: one exposure × one mediator
# =============================================================================
run_cmaverse_mediation_v6 <- function(data, exposure, mediator,
                                      confounders,
                                      time_var  = "hf_time_years",
                                      event_var = "hf_event",
                                      nboot = 500,
  ...
  # --- Prepare data with dummies (no pre-built Surv column) ---
  ...
  # FIX (e): Use actual median of mediator in this complete-case subset
  mediator_median <- stats::median(d[[mediator]], na.rm = TRUE)

  # FIX (d): Try "paramfunc" first, then "imputation" as fallback
  ...
  for (est_method in estimation_methods) {
    res <- tryCatch({
      ...
      CMAverse::cmest(
        ...
        outcome    = time_var,                 # v6 fix: time column name (not Surv())
        event      = event_var,                # v6 fix: event indicator column name
        exposure   = exposure,
        mediator   = mediator,
        basec      = basec,                    # FIX (b): all numeric, no factors
        EMint      = TRUE,
        mreg       = list("linear"),
        yreg       = "coxph",                  # v6 fix: was "survCox" (invalid)
        astar      = 0,                        # reference exposure level
        a          = 1,                        # active exposure level
        mval       = list(mediator_median),    # FIX (e): actual median
        estimation = est_method,               # FIX (d): paramfunc first
        inference  = "bootstrap",
        nboot      = nboot
      )
    }, error = function(e) {
  ...
  # If both methods failed, return with error message (FIX c)
  if (is.null(res)) {
    return(tibble(
      exposure    = exposure,
      mediator    = mediator,
  ...
  # --- Extract summary ---
  s <- tryCatch(summary(res), error = function(e) {
    err_msg <<- paste0("summary() error: ", conditionMessage(e))
    NULL
  })
  ...
  # Row names: "pnde", "tnie", "tnde", "pnie", "te", "pm"
  ef <- tryCatch(as.data.frame(s$effect.decomposition), error = function(e) data.frame())

  # Robust column-name matching (CMAverse versions differ)
  safe_extract <- function(rowname, possible_colnames) {
    if (!rowname %in% rownames(ef)) return(NA_real_)
    for (cn in possible_colnames) {
      if (cn %in% colnames(ef)) {
        val <- as.numeric(ef[rowname, cn])
  ...
  est_cols <- c("Estimate", "estimate", "Effect")
  lo_cols  <- c("2.5%", "lower", "Lower")
  hi_cols  <- c("97.5%", "upper", "Upper")
  p_cols   <- c("Pr(>|z|)", "p.value", "P(>|z|)")
  ...
  tibble(
    ...
    n           = nrow(d),
    events      = sum(d[[event_var]], na.rm = TRUE),
    nde_est     = safe_extract("pnde", est_cols),
    nde_lcl     = safe_extract("pnde", lo_cols),
    nde_ucl     = safe_extract("pnde", hi_cols),
    nde_p       = safe_extract("pnde", p_cols),
    nie_est     = safe_extract("tnie", est_cols),
    nie_lcl     = safe_extract("tnie", lo_cols),
    nie_ucl     = safe_extract("tnie", hi_cols),
    nie_p       = safe_extract("tnie", p_cols),
    te_est      = safe_extract("te", est_cols),
    te_lcl      = safe_extract("te", lo_cols),
    te_ucl      = safe_extract("te", hi_cols),
    pm_est      = safe_extract("pm", est_cols),
    pm_lcl      = safe_extract("pm", lo_cols),
    pm_ucl      = safe_extract("pm", hi_cols),
    status      = "success",
    error_msg   = NA_character_
  )
}
...
# Confounders to pass — same base_covars from Section 8
...
# =============================================================================
cat("\n--- CMAverse v6: Black vs White, through each inflammatory biomarker ---\n")
...
# =============================================================================
cat("\n--- CMAverse v6: Non-White vs White, through each inflammatory biomarker ---\n")
...
# Also print any model errors with their captured messages for debugging
...
evalue_results <- mediation_all %>%
  ...
  dplyr::mutate(
    ...
    evalue_point = tryCatch({
      # Ensure HR is >= 1 for evalues.HR (flip if protective)
      ...
      ev <- EValue::evalues.HR(
        ...
        lo   = lo_for_eval,
        hi   = hi_for_eval,
        rare = TRUE   # rare disease assumption for HF incidence ~7%
      )
...
cat("Section 18 outputs (CMAverse mediation, v6 rewrite):\n")
cat("  - mesa_hf_causal_mediation_cmaverse.csv  (now includes error_msg column)\n")
