...
# =============================================================================
# SECTION 18 (v5 REWRITE): Formal Causal Mediation via CMAverse
# =============================================================================
# Reference: Shi B, Choirat C, Coull BA, VanderWeele TJ, Valeri L (2021).
#            CMAverse: A suite of functions for reproducible causal mediation
#            analyses. Epidemiology, 32(5), e20-e22.
#
# v4 FAILURE ROOT CAUSES (all three fixed below):
#   (a) cmest() with yreg="survCox" requires the outcome to be a pre-built
#       Surv() column in the data frame; passing separate outcome= and event=
#       strings fails silently in many CMAverse versions.
...
#
# v5 FIXES:
#   FIX (a): Pre-build a Surv() column named "surv_outcome" and pass that
#            single column name as outcome= to cmest(). Do NOT pass event=.
...
#            estimator and fall back to "imputation" only if needed. This is
#            more stable for survCox with bootstrap.
#   FIX (e): Set mval to list(median) of the mediator instead of list(0) in
#            case z-score centering produced non-zero medians in subsets.
# =============================================================================

cat("\n========== Section 18 (v5): Formal Causal Mediation (CMAverse) ==========\n")
...
# =============================================================================
# FIX (a): Prepare data with pre-built Surv() column
# =============================================================================
# CMAverse's cmest() internally calls model.frame() which needs the outcome
# column to already be of class Surv when yreg = "survCox".
# We create this column in the data frame BEFORE calling cmest().
...
prepare_mediation_data <- function(data, exposure, mediator, confounders,
  ...
  # FIX (a): Build Surv() column in the data frame
  d$surv_outcome <- survival::Surv(
    time  = d[[time_var]],
    event = d[[event_var]]
  )
...
# =============================================================================
run_cmaverse_mediation_v5 <- function(data, exposure, mediator,
                                      confounders,
                                      time_var  = "hf_time_years",
                                      event_var = "hf_event",
                                      nboot = 500,
  ...
  # --- Prepare data with dummies + Surv column ---
  ...
  for (est_method in estimation_methods) {
    res <- tryCatch({
      ...
      CMAverse::cmest(
        ...
        outcome    = out_col,                  # FIX (a): pre-built Surv column
        exposure   = exposure,
        mediator   = mediator,
...
# =============================================================================
cat("\n--- CMAverse v5: Black vs White, through each inflammatory biomarker ---\n")
...
# =============================================================================
cat("\n--- CMAverse v5: Non-White vs White, through each inflammatory biomarker ---\n")
...
cat("Section 18 outputs (CMAverse mediation, v5 rewrite):\n")
